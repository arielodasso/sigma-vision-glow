...
interface KnowledgeHelpModalProps {
  article: HelpArticle | null;
  onClose: () => void;
  onSuccess: () => void;
}

const KnowledgeHelpModal = ({ article, onClose, onSuccess }: KnowledgeHelpModalProps) => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (article) {
        const { error } = await supabase.from("help_docs").update(payload).eq("id", article.id);
        ...
        >
          <div className="flex items-center justify-between p-5 border-b border-foreground/[0.06]">
            <h2 className="font-display text-lg font-semibold text-foreground">
              {article ? "Editar artículo de ayuda" : "Nuevo artículo de ayuda"}
            </h2>
            <button
              onClick={onClose}
...
export default KnowledgeHelpModal;