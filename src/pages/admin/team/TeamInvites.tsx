...
import { Plus, Search, Edit, Trash2, Mail, Loader2, Send, Key, User, Shield, Copy } from "lucide-react";
...
const TeamInvites = () => {
  ...
  const fetchInvites = async () => {
    ...
    const { data } = await supabase
      ...
      .select(`
        *,
        inviter:profiles!team_invites_invited_by_fkey(email, full_name)
      `)
  ...
  const filtered = invites.filter((i) =>
    ...
    i.inviter?.email.toLowerCase().includes(search.toLowerCase())
  );
  ...
  const handleResend = async (invite: TeamInvite) => {
    try {
      const inviteUrl = `${window.location.origin}/auth/accept-invite/${invite.token}`;
      ...
      if (RESEND_API_KEY) {
        await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            ...
            to: [invite.email],
            subject: `Invitación a Sigma Tecnologías - ${roleLabels[invite.role] || invite.role}`,
            html: `
              <div style="font-family: system-ui; max-width: 600px; margin: 0 auto; background: #0B0D10; color: #fff; border-radius: 16px; overflow: hidden;">
                ...
                <div style="padding: 32px;">
                  <h2 style="margin: 0 0 24px; font-size: 22px; color: #fff;">Invitación al equipo</h2>
                  <p style="color: #fff; line-height: 1.6;">Hola,</p>
                  <p style="color: #fff; line-height: 1.6;">Has sido invitado a unirte al equipo de <strong>Sigma Tecnologías</strong> como <strong>${roleLabels[invite.role] || invite.role}</strong>.</p>
                  <div style="text-align: center; margin: 32px 0;">
                    <a href="${inviteUrl}" style="background: #E2FC03; color: #000; padding: 16px 32px; border-radius: 8px; text-decoration: none; font-weight: bold; display: inline-block;">Aceptar invitación</a>
                  </div>
  ...
  const copyInviteUrl = (token: string) => {
    const url = `${window.location.origin}/auth/accept-invite/${token}`;
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <title>Equipo · Invitaciones · Admin</title>
      </Helmet>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        >
          <div>
            <h1 className="font-display text-3xl font-bold text-gradient">Invitaciones de Equipo</h1>
            <p className="text-sm text-muted-foreground mt-1">Invita nuevos miembros al equipo</p>
          </div>
          <button
            onClick={openCreate}
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-foreground/[0.06] text-left text-xs font-semibold text-foreground/40 uppercase tracking-wider">
                    ...
                    <th className="p-4 hidden lg:table-cell">Invitado por</th>
                ...
                <tbody className="divide-y divide-foreground/[0.04]">
                  {filtered.map((invite) => (
                    <tr key={invite.id} className="hover:bg-foreground/[0.02] transition-colors">
                      ...
                      <td className="p-4">
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${roleColors[invite.role] || "bg-gray-500/20 text-gray-400"}`}>
